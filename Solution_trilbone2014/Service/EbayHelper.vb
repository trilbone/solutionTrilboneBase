...
Friend Class EbayHelper
    ...
    Friend Shared Function GetSingleItem(itemID As String) As XElement
        ...
        Dim itemquery As String = String.Format("{0}callname=GetSingleItem&responseencoding=XML&appid={1}&siteid=0&version=515&ItemID={2}&IncludeSelector=Description,ItemSpecifics,Details", itemendpoint, APP_ID, itemID)
        Dim root As XElement = XElement.Load(itemquery)


        Dim index As Integer

        Dim errorsEl = root.Descendants(NS + "error")
...
Friend Class EbaySearchCriteria
    ...
    Public Property OutputSelector As OutputSelectorType()
        Get
            Return m_OutputSelector
        End Get
        Set(value As OutputSelectorType())
            m_OutputSelector = value
        End Set
    End Property
    Private m_OutputSelector As OutputSelectorType()
    ...
    Public Overrides Function ToString() As String
        ...
        If Not OutputSelector Is Nothing Then
            'outputSelector(0)=SellerInfo
            If Not OutputSelector.Length = 0 Then
                For i = 0 To OutputSelector.Length - 1
                    ret += String.Format("&outputSelector({0})={1}", i, [Enum].GetName(GetType(OutputSelectorType), OutputSelector(i)))
                Next
            End If
        End If
