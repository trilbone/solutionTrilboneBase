...
Public Class clsPage
    ...
    Public Overrides Function ToString() As String
        Return "maxDeviceWidth=" & Me.oDeviceMaxWidth & "; maxDeviceHeight=" & Me.oDeviceMaxHeigh
    ...
    Private Class Row
        ...
        Public Function AddLabel(label As Label) As Boolean
            ...
            If (_currentByRow.X + label.LabelSize_mm.Width) > oMaxSize.Width Then
                ...
                'пробуем ужать
                If label.StartPositionByPage.X < 10 AndAlso ((_currentByRow.X + label.LabelSize_mm.Width) - oMaxSize.Width) < (label.LabelSize_mm.Width * 0.25) Then
                    label.Zoom(oMaxSize.Width)
                    GoTo ex
                End If
...
ex:
    ...
    Private Class Label
        ...
        Public Sub Zoom(newWidth As Integer)
            Dim _koeff = oLabelSize.Width / newWidth
            'Me.oImage = oImage.GetThumbnailImage(newWidth, oImage.Height / _koeff, AddressOf fhg, System.IntPtr.Zero)
            Me.oLabelSize = New SizeF(Math.Round(oLabelSize.Width / _koeff, 1), Math.Round(oLabelSize.Height / _koeff, 1))
        End Sub
        ...
        Private Function fhg() As Boolean
            Return False
        End Function
